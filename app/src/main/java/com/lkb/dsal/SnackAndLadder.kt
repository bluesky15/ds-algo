...
object SnakesLadder {
	...
	fun getMinDiceThrows(move: IntArray, n: Int): Int {
		val visited = IntArray(n)
		val q: Queue<qentry> = LinkedList()
		var qe = qentry()
		qe.v = 0
		qe.dist = 0
		...
		while (!q.isEmpty()) {
			...
			while (j <= v + 6 && j < n) {
				...
				if (visited[j] == 0) {
					...
					if (move[j] != -1) a.v = move[j] else a.v = j
					q.add(a)
				}
				++j
			}
	...
	internal class qentry() {
		var v // Vertex number
				= 0
		var dist // Distance of this vertex from source
				= 0
	}
}
